...
# ── AI-generated audio must never become training data ────────────────


def test_ai_generated_status_is_excluded():
    """Training on our own output would teach the model its own faults."""
    with pytest.raises(RightsError, match="not acceptable"):
        validate_rights(_rights(status=RightsStatus.AI_GENERATED), has_lyrics=True, has_vocals=True)


@pytest.mark.parametrize(
    "source",
    [
        "acestep benchmark output",
        "ace-step generated track",
...
def test_generated_provenance_refused_even_if_claimed_original(source):
    """The repo holds 100+ generated files; a mislabel must not admit them."""
    record = _rights(status=RightsStatus.ORIGINAL_WORK, source=source)
    with pytest.raises(RightsError, match="prohibited provenance"):
        validate_rights(record, has_lyrics=True, has_vocals=True)


def test_legitimate_studio_source_still_accepted():
    """The guard must not block real work — 'studio' contains 'udio'."""
    validate_rights(
